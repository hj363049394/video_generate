...
| 形态 | 用户角色 | 计费模式 | 适合阶段 |
|---|---|---|---|
| **自用生产工具** | 旅游博主本人 | 自付 API 成本（单篇约 1-3 元） | Phase 1（当前） |
| **代运营服务** | MCN/运营者 | 按月/按篇收费给客户提供代写服务 | Phase 1.5 |
| **内容生产 SaaS** | 多租户内容创作者 | 订阅制 + 用量阶梯计费 | Phase 2-3 |

### 2.5.2 典型业务流程（代运营场景）

```
① 客户报需求："下周想要 3 篇带娃赏秋类笔记"
     ↓
② 运营者用 bot 在微信对话内拉取选题：/选题
     ↓
③ bot 返回 Top 5 选题，运营者筛选：/确认 1 /确认 3 /确认 4
     ↓
④ bot 自动生产 3 篇笔记，每篇约 12 分钟
     ↓
⑤ 运营者收到 12 张图文卡 + 3 个视频
     ↓
⑥ 人工 review 后发布到客户的小红书账号
     ↓
⑦ 一周后红狐 get_work 回采数据 → 改进选题策略
```

### 2.5.3 变现路径与单位经济学

| 阶段 | 变现方式 | 单篇成本 | 单篇定价 | 毛利率 |
|---|---|---|---|---|
| Phase 1 | 自用，省下外包写稿费 | 1-3 元（API） | 0（自用） | — |
| Phase 1.5 | 代运营套餐（10 篇/月起） | 1-3 元 | 50-100 元/篇 | 95%+ |
| Phase 2 | SaaS 订阅（100 篇/月起） | 1-3 元 | 20-50 元/篇 | 90%+ |
| Phase 3 | API 开放（按调用计费） | 1-3 元 | 5-15 元/次 | 60%+ |

### 2.5.4 业务边界（不做什么）

- ❌ 不做"完全无人工 review"的自动发布（合规风险）
- ❌ 不做搬运洗稿（同构异题是仿写 ≠ 抄袭，原创度 < 0.30 红线）
- ❌ 不做矩阵号自动化（单 bot 单用户隔离，避免被识别为营销号矩阵）
- ❌ 不做非旅游赛道（人设、提示词、SKILL 全部按旅游行程规划师调优）
